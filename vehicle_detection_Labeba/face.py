...
while True:
    ...
    for (x,y,w,h) in cars:
        ...
        range="safe"
        if (distance < 180 and (direc=="left" or direc=="straight")):
            range="inRange"
        cv2.putText(img2, "face      "+str(format(distance, '.2f'))+"    "+str(format(degree, '.2f'))+"  "+direc+"  "+range,(p,q),fontFace=cv2.FONT_HERSHEY_PLAIN,fontScale=1,color=(0,0,0))
        cv2.putText(img, "face",(x+2,y+2),fontFace=cv2.FONT_HERSHEY_PLAIN,fontScale=1,color=(255, 255, 255)) 
    ...
    cv2.imshow('OurInput', img)
    cv2.imshow('Dataset', img2)
